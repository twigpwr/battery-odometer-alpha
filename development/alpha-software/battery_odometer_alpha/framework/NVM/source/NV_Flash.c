 ...
 */
NVM_STATIC osaTaskId_t mNvIdleTaskId = NULL;
...
)
{
    ...
    else
    {
        ...
        datasetSize = pNVM_DataTable[tableEntryIdx].ElementsCount * pNVM_DataTable[tableEntryIdx].ElementSize;

        /* compute record size */
        if(tblIndexes->saveRestoreAll)
        {
            recordSize = datasetSize;
        }
        else
        {
        ...
        if(flexMetaInfo.rawValue == gNvFlexGuardValue_c) /* no meta found for this table entry ID */
        {
            ...
            {
                /* set dataset offset related to FlexRAM start address */
                flexMetaInfo.fields.NvDataOffset = gFlexNvmDriver.ftfxConfig.flexramTotalSize - datasetSize;
                /* set last meta info address */
                lastFlexMetaInfoAddress = gFlexNvmDriver.ftfxConfig.flexramBlockBase;
            }
            else /* FlexRAM is not empty */
            {
                #if FSL_FEATURE_FLASH_IS_FTFL
                /* wait for EEPROM system to be ready */
                while(!(REG_BIT_GET(gFlexNvmDriver.ftfxRegBase + FTFx_SSD_FCNFG_OFFSET, FTFx_SSD_FCNFG_EEERDY)));
                ...
                /* compute dataset offset */
                flexMetaInfo.fields.NvDataOffset = lastFlexMetaInfo.fields.NvDataOffset - datasetSize;
            ...
            /* set destination address where the record will be written and the source data pointer */
            if(tblIndexes->saveRestoreAll)
            {
                destRecordAddress = gFlexNvmDriver.ftfxConfig.flexramBlockBase + flexMetaInfo.fields.NvDataOffset;
                pData = (uint8_t*)(pNVM_DataTable[tableEntryIdx].pData);
            }
            else
            {
                destRecordAddress = gFlexNvmDriver.ftfxConfig.flexramBlockBase + flexMetaInfo.fields.NvDataOffset + ((tblIndexes->elementIndex) * pNVM_DataTable[tableEntryIdx].ElementSize);
                pData = (uint8_t*)((uint8_t*)(pNVM_DataTable[tableEntryIdx]).pData) + (tblIndexes->elementIndex * pNVM_DataTable[tableEntryIdx].ElementSize);
            }
            ...
            /* write record */
            if(kStatus_FLASH_Success != EEEWrite(&gFlexNvmDriver, destRecordAddress, recordSize, pData))
            {
                status = gNVM_RecordWriteError_c;
            }
            else
            ...
            #if FSL_FEATURE_FLASH_IS_FTFE
            /* write record */
            if(kStatus_FLASH_Success != FLEXNVM_EepromWrite(&gFlexNvmDriver, destRecordAddress, pData, recordSize))
            {
                status = gNVM_RecordWriteError_c;
            }
            else
